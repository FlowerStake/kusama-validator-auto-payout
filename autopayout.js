...
const main = async () => {
  ...
  console.log("\n\x1b[45m\x1b[1m Kusama Network auto payout \x1b[0m\n");
  ...
  if (password) {
    ...
    for (era; era < activeEra; era++) {
        overview = await api.query.staking.erasStakersOverview(era, validator);
        if (overview !== null && overview.isSome) {
           active = overview.unwrap().pageCount;
           if ( active ) {
              procesed = (await api.query.staking.claimedRewards(era, validator)).length
                 if ( procesed == 0 ) {
